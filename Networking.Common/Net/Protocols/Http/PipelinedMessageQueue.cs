...
using Networking.Common.Net.Protocols.Http.External;
...
namespace Networking.Common.Net.Protocols.Http {
    ...
    public class PipelinedMessageQueue : IMessageQueue {
        private readonly ConcurrentPriorityQueue<int, object> _queue = new ConcurrentPriorityQueue<int, object>();
        ...
        public void Enqueue(object message) {
            ...
            if (header == null)
                throw new InvalidOperationException("PipelinedMessageQueue requires the header '" + HttpMessage.PipelineIndexKey + "' to support HTTP pipelinging.");

            var value = 0;
            if (!int.TryParse(header, out value))
                throw new InvalidOperationException("PipelinedMessageQueue require the header '" + HttpMessage.PipelineIndexKey + "' and that it contains a numerical value.");
