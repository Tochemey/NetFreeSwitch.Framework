...
namespace Networking.Common.Net.Protocols.Http {
    ...
    /// <remarks>
    ///     <para>
    ///         Will produce <see cref="HttpRequestBase" /> unless you change the <see cref="BodyDecoder" /> property, which
    ///         will make the listener produce <see cref="HttpRequest" /> instead.
    ///     </para>
    /// </remarks>
    public class HttpListener : ChannelTcpListener {
        ...
        /// </summary>
        public HttpListener() {
            var config = new ChannelTcpListenerConfiguration(() => BodyDecoder == null ? new HttpMessageDecoder() : new HttpMessageDecoder(BodyDecoder), () => new HttpMessageEncoder());
