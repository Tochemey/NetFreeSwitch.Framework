    ...
    public class RangeCollection : IEnumerable<Range> {
        ...
        public Range this[int index]
        {
            get
            {
                if (index < 0
                    || index >= Count)
                    throw new ArgumentOutOfRangeException("index", index, string.Format("Must be between 0 and {0}.", Count - 1));
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerator<Range> GetEnumerator() { return _ranges.GetEnumerator(); }
      ...
      - Several legal but not canonical specifications of the second 500
         ...
         bytes=500-700,601-999*/
            ...
            foreach (var range in ranges) {
                ...
                if (TotalLength >= streamLength)
                    throw new ArgumentException(string.Format("Inner stream is just {0} bytes long, while we should send {1} bytes.", streamLength, TotalLength));
            }
        }
        ...
        ///     <c>bytes X/Y</c>
        /// </returns>
        public string ToHtmlHeaderValue(int streamLength) { return string.Format("bytes {0}/{1}", _ranges[0], streamLength); }
    }
}