...
namespace Networking.Common.Net.Protocols.Http.Authentication.Digest {
    /// <summary>
    ///     Monitors all nonces.
    /// </summary>
    public class NonceService : IDisposable {
        ...
        /// </summary>
        public NonceService() { _timer = new Timer(Sweep, null, _expiresTimeout, _expiresTimeout); }
        ...
        /// <remarks>
        ///     Valid Time span is configured in the <see cref="NonceService(System.TimeSpan)" /> constructor. Default time is
        ///     15 seconds.
        /// </remarks>
        ...
        private void Sweep(object state) {
            _items.Where(kvp => (DateTime.Now - kvp.Value.LastUpdate) > _expiresTimeout).Select(kvp => kvp.Key).ToList().ForEach(key => {
                Nonce item;
