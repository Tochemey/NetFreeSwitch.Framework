...
using Networking.Common.Net.Protocols.Http.Authentication.Digest;
...
namespace Networking.Common.Net.Protocols.Http.Authentication {
    ...
    public class DigestAuthenticator : IAuthenticator {
        ...
        ///     digest
        /// </example>
        public string Scheme { get { return "digest"; } }
        ...
        public static string Encrypt(string realm, string userName, string password, string method, string uri, string qop, string nonce, string nc, string cnonce) {
            ...
            if (qop != null) unhashedDigest = string.Format("{0}:{1}:{2}:{3}:{4}:{5}", ha1, nonce, nc, cnonce, qop, ha2);
            else unhashedDigest = string.Format("{0}:{1}:{2}", ha1, nonce, ha2);
        ...
        /// <returns></returns>
        public static string GetHa1(string realm, string userName, string password) { return GetMd5HashBinHex(string.Format("{0}:{1}:{2}", userName, realm, password)); }
        ...
        protected virtual string Encrypt(string ha1, string ha2, string qop, string nonce, string nc, string cnonce) {
            string unhashedDigest;
            if (qop == "auth-int"
                || qop == "auth") unhashedDigest = string.Format("{0}:{1}:{2}:{3}:{4}:{5}", ha1, nonce, nc, cnonce, qop, ha2);
            else unhashedDigest = string.Format("{0}:{1}:{2}", ha1, nonce, ha2);
        ...
        public string AuthenticationScheme { get { return "digest"; } }
        ...
        public IAuthenticationUser Authenticate(IHttpRequest request) {
            ...
            if (!_nonceService.IsValid(parameters["nonce"], nc)
                && !DisableNonceCheck)
            ...
            var ha2 = GetMd5HashBinHex(a2);
            var hashedDigest = Encrypt(ha1, ha2, parameters["qop"], parameters["nonce"], parameters["nc"], parameters["cnonce"]);

            //validate
            if (parameters["response"] == hashedDigest) return user;
