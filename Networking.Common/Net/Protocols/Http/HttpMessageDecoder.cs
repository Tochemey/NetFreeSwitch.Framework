...
namespace Networking.Common.Net.Protocols.Http {
    ...
    public class HttpMessageDecoder : IMessageDecoder {
        ...
        public HttpMessageDecoder() {
            _headerParser = new HeaderParser {HeaderParsed = OnHeader, RequestLineParsed = OnRequestLine, Completed = OnHeaderParsed};
        ...
        private void OnHeader(string name, string value) { _message.AddHeader(name, value); }

        private void OnHeaderParsed() { _isHeaderParsed = true; }
        ...
        private void OnRequestLine(string part1, string part2, string part3) {
            if (part1.StartsWith("http/", StringComparison.OrdinalIgnoreCase)) {
                ...
                if (!int.TryParse(part2, out code))
                    throw new BadRequestException(string.Format("Second word in the status line should be a HTTP code, you specified '{0}'.", part2));
                ...
                if (!part3.StartsWith("http/", StringComparison.OrdinalIgnoreCase))
                    throw new BadRequestException(string.Format("Status line for requests should end with the HTTP version. Your line ended with '{0}'.", part3));

                _message = _messageSerializer != null ? new HttpRequest(part1, part2, part3) : new HttpRequestBase(part1, part2, part3);
            }
        }
        ...
        private void TriggerMessageReceived(HttpMessage message) {
            ...
            if (_messageSerializer != null
                ...
                if (cookies != null) request.Cookies = _cookieParser.Parse(cookies);
            }
