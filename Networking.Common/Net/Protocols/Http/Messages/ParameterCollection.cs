...
namespace Networking.Common.Net.Protocols.Http.Messages {
    ...
    public class ParameterCollection : IParameterCollection {
        ...
        /// </returns>
        public override string ToString() { return string.Join(",", _items.Values.Select(x => string.Format("{0}={1}", x.Name, x.Value))); }
        ...
        public IEnumerator<IParameter> GetEnumerator() { return _items.Values.GetEnumerator(); }
        ...
        public static void Parse(string value, ParameterCollection target) {
            ...
            if (name != "") target.Add(name, value.Substring(oldPos).Trim(' ', '"'));
        }
