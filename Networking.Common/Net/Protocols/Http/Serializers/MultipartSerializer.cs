...
using Networking.Common.Net.Protocols.Serializers.Mono;

namespace Networking.Common.Net.Protocols.Http.Serializers {
    /// <summary>
    ...
    public class MultipartSerializer : IMessageSerializer {
        ...
        public void Serialize(object source, Stream destination, out string contentType) { throw new NotSupportedException(); }
        ...
        public object Deserialize(string contentType, Stream source) {
            ...
            var result = new FormAndFilesResult {Form = new ParameterCollection(), Files = new HttpFileCollection()};
            ...
            while ((element = multipart.ReadNextElement()) != null) {
                ...
                if (!string.IsNullOrEmpty(element.Filename)) {
                    if (string.IsNullOrEmpty(element.ContentType))
                        throw new DecoderFailureException(string.Format("Value '{0}' lacks a content type.", element.Name));
                    ...
                    var path = string.IsNullOrEmpty(internetCache) ? Path.Combine("var", "tmp") : Path.Combine(internetCache.Replace("\\\\", "\\"), "tmp");
                    ...
                    var file = new HttpFile {Name = element.Name, OriginalFileName = originalFileName, ContentType = element.ContentType, TempFileName = element.Filename};
