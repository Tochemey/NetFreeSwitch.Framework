...
namespace Networking.Common.Net.Protocols.Stomp.Broker.Services {
    ...
    public class TransactionManager : ITransactionManager {
        ...
        public void Begin(string transactionId) {
            ...
            if (_transactions.TryGetValue(transactionId, out transaction)) throw new InvalidOperationException("Transaction '" + transactionId + "' have already been started.");
        ...
        public void Enqueue(string transactionId, Action commitTask, Action rollbackTask) {
            ...
            if (!_transactions.TryGetValue(transactionId, out transaction))
                throw new InvalidOperationException("Transaction '" + transactionId + "' do not exist (or have been committed/rolled back).");
        ...
        /// </summary>
        public void Cleanup() { foreach (var transaction in _transactions) transaction.Value.Rollback(); }

        /// <summary>
        ///     Determines if the connection have active transactions.
        /// </summary>
        public bool HasActiveTransactions { get { return _transactions.Count > 0; } }
    }
}