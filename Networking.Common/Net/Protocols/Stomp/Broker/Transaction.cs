...
namespace Networking.Common.Net.Protocols.Stomp.Broker {
    ...
    public class Transaction {
        private readonly LinkedList<Action> _commitTasks = new LinkedList<Action>();
        private readonly LinkedList<Action> _rollbackTasks = new LinkedList<Action>();
        ...
        /// </summary>
        public void Rollback() { foreach (var task in _rollbackTasks) task(); }

        /// <summary>
        ///     Commit transaction.
        /// </summary>
        public void Commit() { foreach (var task in _commitTasks) task(); }
    }
}