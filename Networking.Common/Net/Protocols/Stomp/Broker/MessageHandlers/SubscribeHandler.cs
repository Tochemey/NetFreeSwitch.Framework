...
namespace Networking.Common.Net.Protocols.Stomp.Broker.MessageHandlers {
    /// <summary>
    ...
    public class SubscribeHandler : IFrameHandler {
        ...
        public IFrame Process(IStompClient client, IFrame request) {
            ...
            var subscription = new Subscription(client, id) {AckType = ackType, QueueName = queue.Name};
        ...
        private static string GetAckType(IFrame request) {
            ...
            switch (ackType) {
                ...
                default:
                    throw new BadRequestException(request, string.Format("Unknown ack type '{0}', must be one of: auto, client, or client-individual.", ackType));
            }
            return ackType;
        }
    }
