    ...
    public class DisconnectHandler : IFrameHandler {
        ...
        public IFrame Process(IStompClient client, IFrame request) {
            ...
            if (client.HasActiveTransactions)
                throw new BadRequestException(request, "Got pending transactions. Just close the socket to abort them, or send proper commits/rollbacks before the DISCONNECT frame..");
