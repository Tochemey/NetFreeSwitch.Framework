    ...
    public class MicroMessageEncoder : IMessageEncoder {
        ...
        public MicroMessageEncoder(IMessageSerializer serializer, IBufferSlice bufferSlice) {
            ...
            if (bufferSlice.Capacity < 520)
                throw new ArgumentOutOfRangeException("bufferSlice", bufferSlice.Capacity, "At least the header should fit in the buffer, and the header can be up to 520 bytes in the current version");
        ...
        private int CreateHeader() {
            ...
            if (_message is Stream) {
                ...
                if (contentType.Length > byte.MaxValue)
                    throw new InvalidOperationException("The AssemblyQualifiedName (type name) may not be larger than 255 characters. Your type: " + _message.GetType().AssemblyQualifiedName);
            }
