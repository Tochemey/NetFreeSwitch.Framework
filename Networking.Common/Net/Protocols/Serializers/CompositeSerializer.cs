...
namespace Networking.Common.Net.Protocols.Serializers {
    ...
    public class CompositeIMessageSerializer : IMessageSerializer {
        ...
        public void Serialize(object source, Stream destination, out string contentType) { throw new NotSupportedException("We can currently not encode outbound message bodies."); }
        ...
        private string GetContentTypeWithoutCharset(string contentType) {
            if (!string.IsNullOrEmpty(contentType)) {
                ...
                if (pos > 0) return contentType.Substring(0, pos).Trim();
            }
