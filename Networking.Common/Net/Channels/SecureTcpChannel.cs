...
namespace Networking.Common.Net.Channels {
    ...
    public class SecureTcpChannel : ITcpChannel {
        ...
        private void OnMessageReceived(object obj) { MessageReceived(this, obj); }

        private void OnReadCompleted(IAsyncResult ar) {
            var readBytes = 0;
            try { readBytes = _stream.EndRead(ar); }
            catch (Exception ex) {
                HandleDisconnect(ex);
            }
            ...
            if (BufferPreProcessor != null) {
                var read = BufferPreProcessor(this, _readBuffer);
        ...
        private void ReadAsync() {
            try { _stream.BeginRead(_readBuffer.Buffer, _readBuffer.Offset, _readBuffer.Capacity, OnReadCompleted, null); }
            catch (Exception e) {
                HandleDisconnect(e);
            }
        }
        ...
        private void SendCurrent() {
            ...
            _encoder.Send(_writeBuffer);
            try { _stream.BeginWrite(_writeBuffer.Buffer, _writeBuffer.Offset, _writeBuffer.Count, OnSendCompleted, null); }
            catch (Exception e) {
                HandleDisconnect(e);
            }
        }
