    ...
    public class PersistentCircularIndex {
        ...
        private void CreateFile(string fileName, int maxDataSize, int maxQueueSize) {
            using (var fs = File.Create(fileName)) {
                ...
                for (var i = 0; i < maxQueueSize; i++) writer.Write(data, 0, maxDataSize);
            ...
            using (var fs = File.Open(_fileName, FileMode.Open)) {
                ...
                lock (_syncLock) {
                    ...
                    if (data.Length > recordSize)
                        throw new ArgumentOutOfRangeException("data", data.Length, "Record size is max " + recordSize + " bytes.");
                    ...
                    if (nextWriteRecord == maxQueueSize) nextWriteRecord = 0;
        ...
        public string Dequeue() {
            using (var fs = File.Open(_fileName, FileMode.Open)) {
                ...
                lock (_syncLock) {
                    ...
                    if (nextReadRecord == maxQueueSize) nextReadRecord = 0;
