...
using Networking.Common.IO.Serializers;

namespace Networking.Common.IO {
    /// <summary>
    ...
    public class OneFilePerItemQueue<T> : IQueue<T> {
        ...
        public OneFilePerItemQueue(string queueName) {
            ...
            DataDirectory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "GriffinFramework");
        }
        ...
        public async Task<T> DequeueAsync() {
            ...
            try {
                ...
                try { using (var stream = File.OpenRead(filename)) return (T) Serializer.Deserialize(stream, typeof (T)); }
        ...
        public async Task EnqueueAsync(T item) {
            ...
            try {
                ...
                try { using (var stream = File.OpenWrite(filename)) Serializer.Serialize(item, stream, typeof (T)); }
                catch {
        ...
        private string GetFileName(int index) { return Path.Combine(_fullPath, index + ".data"); }
    }
}